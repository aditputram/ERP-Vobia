...
class SalesReportRouteTests(TestCase):
    ...
    def test_sales_report_routes_render(self):
        for name in ("sales:product_performance", "sales:pareto", "sales:transactions"):
    ...
    def test_dashboard_custom_date_range_filters_all_sales_metrics(self):
        ...
        self.assertEqual(len(response.context["daily"]), 10)
        self.assertEqual(response.context["daily"][4]["day"], date(2026, 8, 5))
        self.assertEqual(response.context["daily"][4]["label"], "05 Agu")
        self.assertEqual(response.context["daily"][4]["net"], Decimal("100000"))
        self.assertEqual(response.context["daily"][4]["gross"], Decimal("110000"))
        self.assertEqual(response.context["daily"][5]["net"], 0)
    ...
    def test_product_performance_accepts_multiple_source_group_and_category_filters(self):
        ...
        self.assertEqual(response.context["totals"]["orders"], 2)
        self.assertEqual(response.context["totals"]["net"], Decimal("300000"))
        self.assertEqual(response.content.count(b"data-multi-select data-all-label"), 4)
...
class ManualSalesAndRequirementTests(TestCase):
    ...
    def test_requirement_summary_collapses_months_into_one_range_per_marketplace(self):
        ...
        self.assertEqual(
            summarize_import_requirements(requirements),
            [
                {
                    "source": SalesOrder.Source.SHOPEE,
                    "period_start": date(2026, 8, 1),
